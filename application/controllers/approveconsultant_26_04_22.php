...
//		 print_r($response);
	...
	public function addLeadApprovedHours()
	{
		...
		if(!empty($billingRecords)) {
			$totalHours = $billingRecords[0]['totalApprovedHours'];
			$totalLoggedHours = $billingRecords[0]['totalLoggedHours'];
			$this->consultant_timesheet_mod->toggleStoreMonthlyData($taskProjectICode, $taskEmployeeICode, $year, $month, $totalHours, $totalLoggedHours, $update['ModifiedByLead']);
		}		
