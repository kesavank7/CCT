...
//		return $query->result_array();
	...
	public function toEditMonthlyData($taskProjectICode, $taskEmployeeICode, $year, $month, $workedHours, $totalHours, $leadId){
		...
		if(!empty($data)) {
			// $dataParams['WorkedHours'] = $data[0]['WorkedHours'] - $dataParams['WorkedHours'];
			// $dataParams['LeadApprovedHours'] = $data[0]['LeadApprovedHours'] - $dataParams['LeadApprovedHours'];
			// $dataParams['AccountsApprovedHours'] = $data[0]['AccountsApprovedHours'] - $dataParams['LeadApprovedHours'];
			...
			$dataParams['AccountsApprovedHours'] = $this->subtractHoursAndMinutes($data[0]['AccountsApprovedHours'] , $dataParams['LeadApprovedHours']);
			if($dataParams['AccountsApprovedHours'] <= 0) $dataParams['AccountsApprovedHours'] = 0;
			if($dataParams['LeadApprovedHours'] && $dataParams['LeadApprovedHours'] > 0 ) return	$this->updateConsultantMonthlyData($data[0]['id'] ,$dataParams);
			else return	$this->deleteMonthlyHours($data[0]['id']);
		}
	}
	...
	// public function subtractHoursAndMinutes($time_one,$time_two) {
	// 	$time_one = ($time_one == '' || $time_one == '.00') ? '00.00' : $time_one;
	// 	$time_two = ($time_two == '' || $time_two == '.00') ? '00.00' : $time_two;
	// 	$start_t = new DateTime($time_one);
	// 	$current_t = new DateTime($time_two);
	...
	public function subtractHoursAndMinutes($time_one,$time_two) {
		...
		$hh1 = floor($time_one);
		$mm1 = ($time_one-$hh1)*100;			
		$minutes1 += $hh1 * 60;
		$minutes1 += $mm1;
		
		$hh2  = floor($time_two);
		$mm2 = ($time_two-$hh2)*100;			
		$minutes2 += $hh2 * 60;
		$minutes2 += $mm2;*/
			
		
		$time_one = number_format((float)$time_one,2);
		$time_one = ($time_one == '' || $time_one == '.00') ? '00.00' : $time_one;
		$time_two = ($time_two == '' || $time_two == '.00') ? '00.00' : $time_two;
		...
		$hours2 = 0;
		$difference_hours = 0;
		list($hh1, $mm1) = explode('.', strval($time_one));
		$h1 = ($hh1 == '00' || $hh1 == '.00') ? 0 : (int)$hh1;
		$m1 = ($mm1 == '00' || $mm1 == '.00') ? 0 : (int)$mm1;
		
		$minutes1 += $h1 * 60;
		$minutes1 += $m1;

		list($hh2, $mm2) = explode('.', strval($time_two));
		$h2 = ($hh2 == '00' || $hh2 == '.00') ? 0 : (int)$hh2;
		$m2 = ($mm2 == '00' || $mm2 == '.00') ? 0 : (int)$mm2;
		
		$minutes2 += $h2 * 60;
		$minutes2 += $m2;

		/*$difference_hours = floor(($minutes1-$minutes2) / 60);
		$difference_mins = (((($minutes1-$minutes2) / 60)-$difference_hours)*100)*60;
		if($difference_mins < 10) {
				$difference_mins = '0'.$difference_mins;
		}*/
		if(is_int($difference_hours)) {
			$return_time = $difference_hours.".00";
		} else {
			$return_time_diff = explode('.', strval($difference_hours));

			$dif = (number_format(($difference_hours - intval($return_time_diff[0])),2) * 60);
			if( abs($dif) - floor(abs($dif)) >= 0.5 )
            {
				$difference_mins = ceil($dif);
            } else {
				$difference_mins = floor($dif);
            }
			// $difference_mins = ceil(($difference_hours - intval($return_time_diff[0])) * 60);
			$difference_mins_formatted = (strlen((string)$difference_mins) == 1) ? '0'.$difference_mins : $difference_mins;
			$return_time = $return_time_diff[0].".".$difference_mins_formatted;
		}
		return $return_time;
	}
