...
jQuery(document).ready(function($) {
        ...
        jQuery.ajax({
            ...
            if (data) {
                ...
                $.each(data.timesheet_list, function(index, task_detail) {
                    ...
                    rate_per_hr_input =
                        '<input type="number" class="rate_per_hr" name="rate_per_hr[]" value="' +
                        rate_per_hr + '">';
                        approved_hours =
                        ...
                        `">
                        <input type="hidden" id="lead_approved_hours" value="${task_detail.LeadApprovedHours}" class="lead_approved_hours"></td>`;
                    amount_per_day =
                        `<td width="10%">
                            <input type="number" class="amount_per_day" id="amount_per_day" name="amount_per_day[]" value="${ rate_per_hr ? parseFloat(rate_per_hr * approvedHoursForAmountCalc(taskApprovedHour)).toFixed(2) : 0.00}">
                            </td>`;
                    approveCheckbox =
                    ...
                    if (task_detail.IsAccountsApproved) {
                        rate_per_hr_input =
                            '<input type="number" class="rate_per_hr" name="rate_per_hr[]" value="' +
                            rate_per_hr + '" readonly>';
                            approved_hours =
                            `<td width="10%"><input type="text" class="approved_hours" id="approved_hours" name="approved_hours[]" value="` +
                            taskApprovedHour.replace('.',':').slice(0)  +
                            `"  
                        ...
                        amount_per_day =
                            `<td width="10%">
                            <input type="number" class="amount_per_day" id="amount_per_day" name="amount_per_day[]" value="${ rate_per_hr ? parseFloat(rate_per_hr * approvedHoursForAmountCalc(taskApprovedHour)).toFixed(2) : 0.00}" readonly>
                            </td>`;
                        approveCheckbox =
                            '<input type="checkbox" class="accApproveCk" value="' +
                            task_detail.id +
                        ...
                        approveButton =
                            '<button type="button" class="btn btn-sm btn-green acc_edit" id="acc_edit" data-id="' +
                            task_detail.id + '">Edit</button>';
                        if (task_detail.IsHrApproved) {
                            approveButton = 'HR Approved';
                        }
                    }
                    ...
                    appendHtml = '<tr>' +
                        ...
                        '<td width="10%">' + dateFormator(task_detail.CreatedOn) +
                        '</td>' +
                        '<td width="10%" align="right">' + task_detail.WorkedHours
                        .slice(0).replace('.',':') +
                        '</td>' +
                        '<td width="10%" align="right">' + task_detail
                        .LeadApprovedHours.slice(0).replace('.',':') + '</td>' +
                        '<td width="10%">' + rate_per_hr_input + '</td>' +
                        approved_hours +
                        amount_per_day +
                        '<td width="10%">' +
                        approveCheckbox +
                ...
                $('#tbl_list tbody').append(
                    `<tr>
							...
							<td class="bold-16" colspan="5" id="totalHrsApproved" align="right">` + totalHrsApproved + ` Hours</td>
    ...
    $(document).on('click', '.acc_edit', function() {
        $(this).parent().parent().children().eq(6).children().eq(0).attr("readonly", false);
        $(this).parent().parent().children().eq(7).children().eq(0).attr("readonly", false);
        $(this).parent().parent().children().eq(8).children().eq(0).attr("readonly", false);
        $(this).parent().parent().children().eq(9).children().eq(0).attr("readonly", false);

        let editButton = this;
        editButton.textContent = "Delete";
        editButton.classList.remove("acc_edit");
        editButton.classList.remove("btn-green");
        editButton.classList.add("acc_delete");
        editButton.id = "acc_delete";
    ...
    $(document).on('change', '.accApproveCk', function() {
        let LeadApproveHours = $(this).closest('tr').find('.lead_approved_hours').val();
        let accountsApprovedHours = $(this).closest('tr').find('.approved_hours').val();
        if (!accountsApprovedHours.includes(':')) {
            alert("Please enter the Approved Hours in valid format: (00:00).");
            $(this).prop('checked', false);
            return false;
        }
        var arr = accountsApprovedHours.split(':');
        ...
        let leadHour = parseFloat(LeadApproveHours.slice(0)).toFixed(2);
        let accountHour = parseFloat(hoursFormatConversion(accountsApprovedHours)).toFixed(2);
        if(leadHour < accountHour) {
            alert("Approved Hours cannot be greater than Lead Approved Hours.");
            $(this).prop('checked', false);
            return false;
        }
    ...
    $(document).on('change', '.amount_per_day', displayTotals);
