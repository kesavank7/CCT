                        ...
                        <div class="col-md-5">
                            <select class="form-control" id="entry_progress_filter" name="entry_progress_filter"
                                ...
                                <option value="<?php echo $project_detail['ProjectICode']; ?>"
                                    <?php echo ($search_project_id == $project_detail['ProjectICode']) ? ' selected ' : "" ?>>
                            ...
                            <tr id="task_progress_row_<?php echo $progress_detail['TaskICode']; ?>">
                                ...
                                <td data-title="Project Name">
                                    ...
                                    <a href="javascript:void(0)"
                                        ...
                                        id="task_history_details" data-toggle="modal">Tasks Progress</a>
                                </td>
                                ...
                                <td data-title="Estimated&nbsp;/ Pending Hours">
                                    ...
                                    / <br />
                                    <?php
                                        if (isset($progress_detail['EstimatedHours']) && isset($progress_detail['workedHours'])) {
                                            $value = (float)$progress_detail['EstimatedHours'] - (float)$progress_detail['workedHours'];
                                            $txt = "<span ";
                                            if ($value < 0) {
                                                $txt .= " class='red_txt' ";
                                            }
                                ...
                                <td data-title="Logged Hours / Date">
                                    <?php
                                        ...
                                        echo "<p>&nbsp;</p>";
                                        echo "<input type='text' name='TaskProgressDate[" . $progress_detail['TaskICode'] . "][]'   class='form-control datepicker'  max='" . date('Y-m-d') . "' onchange='checkdataforlatereason(" . $progress_detail['TaskICode'] .",". $progress_detail['ProjectICode'].")' id='TaskProgressDate_" . $progress_detail['TaskICode'] . "' value='" . $dat . "' placeholder='dd-mm-yyyy' />";
                                        ?>
                                    <input type="hidden"
                                        id="lead_id_report_<?php echo $progress_detail['TaskICode']; ?>"
                                        value="<?php echo $progress_detail['lead_id']; ?>" />
                                    <input type="hidden"
                                        id="latereasoncheck_<?php echo $progress_detail['TaskICode']; ?>" value="0" />
...
function show_loader(element) {
    jQuery(element).append(
        '<div class="ajax-loader"><div class="overlay"><img src="<?php echo base_url(); ?>public/images/ajax-loader.gif" alt="ajax-loader"></div></div>'
    );
}
...
function checkdataforlatereason(task_id, projectICode) {

    document.getElementById("TaskProgressDate_" + task_id + "").blur();
    document.getElementById("WorkDescription_" + task_id).focus();

    let saveBtn = document.getElementById("disable_check_" + task_id);
    saveBtn.textContent = saveBtn.textContent.replace("Update", "Save");

    let dateEntered = document.getElementById("TaskProgressDate_" + task_id + "").value;
    let dateEnteredArr = dateEntered.split("-");
    let dateToCheck = new Date(dateEnteredArr[2], dateEnteredArr[1] - 1, dateEnteredArr[0]);
    let dateCurrent = new Date();
    dateCurrent.setHours(0, 0, 0, 0);

    if (dateToCheck.getTime() < dateCurrent.getTime()) {
        jQuery.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>cosultantprogress/taskProgressEntryOnThatdate",
            data: {
                'task_code': task_id,
                'prog_date': dateEntered,
            ...
            success: function(res) {
                ...
                if (!Number(res)) {
                    ...
                    saveBtn.textContent = saveBtn.textContent.replace("Save", "Update");
    ...
    if (insert_row == 0) {
        ...
        if (!mans_hour.trim()) {
            ...
            if ((parseInt(arr[0]) <= 0) && (parseInt(arr[1]) <= 0)) {
                ...
                //     return;
                // }
                let man_hour_mins = arr[1].trim();
                var pattern_minutes = /^[0-5][0-9]+$/;
                if (!pattern_minutes.test(man_hour_mins)) {
...
function GetTaskHistoryDetailsByMonth() {
    console.log('PorjectIcode',jQuery('#Time_ProjectICode').val());
    console.log('TaskIcode',jQuery('#Time_TaskICode').val());
    jQuery.ajax({
        type: "POST",
        url: "<?php echo base_url(); ?>cosultantprogress/gettaskhistory",
        data: {
            PorjectIcode: jQuery('#Time_ProjectICode').val(),
            TaskIcode:  jQuery('#Time_TaskICode').val(),
            month: jQuery('#time_sheet_month').val(),
            year: jQuery('#time_sheet_year').val(),
        },
        success: function(res) {
            jQuery('#history_details_append').html(res);
            // jQuery('#task_progress_modal').modal('show');
...
function deleteTaskProgressEntery(taskProgressId, taskProgressDate, projectICode, taskICode) {
    if (confirm('Are you sure you want to delete this task?')) {
        jQuery.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>cosultantprogress/deleteTaskProgressEntery",
            data: {
                task_progress_id: taskProgressId,
                task_progress_entry_date: taskProgressDate
            },

            beforeSend: function() {

                show_loader('.modal-body');
            ...
            success: function(res) {
                ...
                if (res.status == 1) {
                    alert(res.msg);
                    GetTaskHistoryDetails(projectICode, taskICode);
                }
                ...
                <table class="table table-hover">
                    <thead>
                        <tr>
                                ...
                                <div class="type-container monthly-field">
                                    ...
                                    <div class="field-inline">
                                        ...
                                        <select class="form-control" id="time_sheet_month" name="month">
                                            <?php
                                        $selctedMonth = date('F');
                                        echo "<option value='Select Month'  selected disabled >Select Month</option>";
                                        for ($i = 12; $i > 0; $i--) {
                            ...
                            <th width="60%">
                                <div class="timesheet-action clearfix">
                ...
                <table class="table table-hover" id="tbl_list">
                    <thead>
                        <tr>
                            ...
                            <th width="4%;">Hours</th>
                            <th width="10%;">Entered Date and Time</th>
