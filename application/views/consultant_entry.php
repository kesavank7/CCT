	...
	<div class="container">
		<div class="starter-template">
			...
			<div class="add_filter">
				<div id="no-more-tbl">
					<form action="<?php echo base_url()."consultanttask/entry_insert"; ?>" id="task_entry_form" method="post" class="form-horizontal">
								...
								<tr>
									<td data-title="" width="20%">
										<select class="form-control" id="ProjectICode" name="ProjectICode">
											...
											<br>
											<select class="form-control" name="ConsultantICode[]" id="ConsultantICode" multiple>
											<option value=''> Select Consultant </option>
											<?php foreach($consultantList as $consultant){?>
												<option value='<?php echo $consultant['ConsultantICode']; ?>'>
												<?php echo $consultant['ConsultantFirstName']." ".$consultant['ConsultantLastName'];?>
												</option>
											<?php } ?>
		...
		<div class="panel panel-info ovrhid">
			...
			<div class="panel-body">
				<form action="<?php echo base_url()."consultanttask/entry_search"; ?>" id="task_entry_search_form" method="post" class="form-horizontal">
			...
			<!-- Filter --->
				...
				<div class="form-group col-sm-12">
					...
					<!-- Table Entrying -->
					<form action="<?php echo base_url(); ?>consultanttask/entry_update" id="task_entry_edit_form" method='post' class='form-horizontal'>
								...
								<?php } else {
									...
									<tr id="new_task_list_<?php echo $list_val['TaskICode']; ?>">
										...
										<td data-title="Test Scenarios" class="pos-rel task-desc">
											<div class="tbl_row_scroll_entry">
												<?php echo $list_val['ConsultantFirstName'].' '.$list_val['ConsultantLastName'];?>
											</div>
										</td>
...
?>
	...
	*/
	function valid()
	{
		...
		if(!jQuery("#ConsultantICode").val())
		{
			alert("Choose the Consultant...");
			jQuery("#ConsultantICode").focus();
			return false;
		}
	...
	*/
	function inactive_entry(id) {
		if(confirm("Are you sure want to deactivate this task?"))
		{
			window.location.href="<?php echo base_url(); ?>consultanttask/entry_inactive/?request_id=" + id +"&page="+jQuery(".paginate_button.current").html();
		}
	}
	...
	function edit_entry(id, sno){
		if(edit_open == 0)
		{
			...
			jQuery.ajax({
				type: "POST",
				url: "<?php echo base_url(); ?>consultanttask/entry_edit/",
				data: {TaskICode:id},
	...
	<script type="text/javascript">
		// $(function(){
		// 		$('#ConsultantICode').multiselect({
		// 			placeholder: "select consultants",
		// 			tags: true,
		// 			tokenSeperators: ['/',',',','," "]
		// 		});
		// });
		jQuery("#consultanttask_entry").addClass('active');
		jQuery("#TaskDescription").focus();
		...
		jQuery(document).ready(function() {
			...
			jQuery("#ProjectICode").change(function(){
		    	if(jQuery("#ProjectICode").val()){
			        ...
			        jQuery.ajax({
			            type: "POST",
			            url: c_url+"consultanttask/phaseList",
		    ...
		    jQuery("#PhaseTypeICode").change(function(){
		    	if(jQuery("#PhaseTypeICode").val()){
			        ...
			        jQuery.ajax({
			            type: "POST",
			            url: c_url+"consultanttask/taskTypeList",
